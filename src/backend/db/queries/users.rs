...
pub async fn search_users_by_prefix(
    ...
    sqlx::query_as::<_, User>(&format!(
        "{} FROM users WHERE LOWER(username) LIKE ? AND deleted_at IS NULL ORDER BY username LIMIT ?",
        SQL_SELECT_USER_FIELDS
    ))
    .bind(pattern)
    .bind(limit)
...
pub async fn search_users_excluding_self(
    ...
    sqlx::query_as::<_, User>(&format!(
        "{} FROM users WHERE LOWER(username) LIKE ? AND deleted_at IS NULL AND id != ? ORDER BY username LIMIT ?",
        SQL_SELECT_USER_FIELDS
    ))
    .bind(pattern)
    .bind(requester_id)
